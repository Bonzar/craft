...
### Согласен ли грейдер с человеком

Ассерты калибруются на тех же ответах, где ищутся дефекты, — проверка настроена
по данным, которые сама и объясняет. Поэтому есть отдельная сверка: 28 записанных
ответов агента размечены человеком (`fixtures/labels/incident-closure.tsv`) —
разбор доведён, разбор не закрыт, работа не сделана. Вердикт человека ставился до
просмотра машинного и закоммичен раньше сравнения: порядок виден в истории.

Совпало 27 из 28. Из двух исходных расхождений одно оказалось ошибкой разметчика
(ступень была названа прямо, я её не увидел), одно осталось: слово «ступень»
попало в цитату, а решение не названо — текстовый ассерт этого не различает.
Подкручивать ассерт под единственный случай не стал: это подгонка, а не точность.

Сверка гоняется в самотесте и падает, если появится **новое** расхождение —
то есть будущая правка грейдера, разошедшаяся с человеческой оценкой, всплывёт
сразу. В CI шаг пропускается с отметкой `SKIP`: сохранённых прогонов там нет.
...
Прогоны идут пачками — по умолчанию четыре разом, меняется через `EVAL_JOBS`.
Замер из двенадцати прогонов последовательно занимал 55 минут чистого времени,
пачками — около четверти часа. Больше четырёх ставить не стоит без нужды:
упрёмся в ограничения API и получим ретраи вместо ускорения.

Ход виден на странице `evals/runs/<run_id>/progress.html` — она сама
обновляется и показывает каждый прогон, его состояние и длительность. Путь к ней
раннер печатает при старте. В терминал идёт только итоговая таблица: живая
строка не переживает ни параллельность, ни запись в файл при фоновом запуске.

Сам раннер проверяется без моделей: `bash evals/fake-run.sh` подсовывает
подставной прогон с записанным потоком и сверяет, что пачки быстрее
последовательного, сводка сходится с отчётом, строки отчёта целы, а страница
доходит до готовности. Гоняется в CI.
