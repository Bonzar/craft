...
(для скилла разбора это `incident doc cached`), `rule_trigger_hook` — путь к
хуку, чьё срабатывание на промпте проверяется до старта. Нет улики в потоке →
ERROR вместо вердикта; промпт не запускает подачу правила → кейс бракуется.
Без этого замер меряет память модели о правиле, а не его соблюдение — на этом
уже был разбор.
