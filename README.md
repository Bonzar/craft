...
- **запись через Bash** — перенаправление, `tee`, правка на месте, запись из
  интерпретатора, `cp`/`mv`. Гейтится ЦЕЛЬ, а не команда: путь отклоняется,
  только если он не эфемерный и не игнорируется гитом. Игнор гитом и есть
  различитель сборки — вывод сборки, покрытие и зависимости лежат в
  игнорируемых путях, исходник нет. Непокрыто: неопознанная конструкция записи,
  пакетные менеджеры и операции гита над рабочим деревом, перенаправление в
  закавыченную цель (кавычки вычёркиваются, чтобы `>` в сравнении не считалось
  записью).
...
`universal-mark-plan-file.sh`. Одобрение критика самоинвалидирующееся по
устройству — отметка стоит на хеше файла, а работа критика в том, чтобы выдать
замечания, выполнение которых этот хеш меняет. Условие завершения машинное:
после трёх завершённых прогонов по одному разговору гейт пропускает показ, как и
требует правило «Плато». Счётчик обнуляет реплика Влада.
Отметка живёт до правки плана: reset нового хода
...
Четвёртый контур — `universal-guard-plan-service-turn.sh`: в ходе, начатом
служебным сообщением, план не показывается ПОВТОРНО. План-режим при отсутствии
Влада закрывается сам, следом приходит техническое продолжение хода — повтор
закроется так же. Различитель повтора — хеш файла плана: гейт запоминает его при
пропуске и отклоняет только совпадение, поэтому исправленный после замечания
план проходит. Метку «ход служебный» ставит и снимает
`universal-plan-gate-reset.sh` по словарю якорей (`service-anchors.txt`); реплика
Влада снимает обе метки.

Пятый — `universal-guard-plan-exit-failure.sh` на провал показа плана: ошибка
вызова значит, что план не показывался, а системный текст о выходе из
план-режима легко читается как разрешение. Хук дописывает в контекст обратное
раньше, чем агент увидит ошибку. Вернуть режим он не может — permission mode
доступен хукам только на чтение.
