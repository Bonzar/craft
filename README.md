...
`universal-mark-plan-critic.sh` по `subagent_type`, путь плана запоминает
`universal-mark-plan-file.sh`. Отметка живёт до правки плана: reset нового хода
её не трогает. Обхода по `CRAFT_AUTONOMOUS` у этого гейта нет: автономный прогон
планов не показывает.

Третий контур — `universal-guard-plan-delta.sh`: он не даёт показать план, где
часть юнитов уже одобрена в этой сессии, а часть новая. Один файл на две роли —
до показа сверяет хеши сущностных юнитов с накопителем, после одобрения
дописывает их туда. Перепоказ плана целиком и план без повторов проходят;
`PLAN_DELTA=off` глушит гейт.
