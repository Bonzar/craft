...
func TestNameMatchesGuardsAgainstNeighbourCinema(t *testing.T) {
	...
	// Названия одной площадки в ЕАИС и в OSM расходятся родовыми словами.
	for _, c := range [][2]string{
		{"Иллюзион", "Иллюзион"},
...
// Второй дефект того же класса, вскрытый живым прогоном: по «Pushka Mitino»
// Photon отдаёт объект с именем «Pushka», по «Pushka Brateevo» — его же. Три
// площадки сети получили одну точку, и выглядела она достоверной.
//
// Отсюда правило: отбрасывать при сверке можно только родовые слова
// («кинотеатр», «летний»), но не различающие — топоним площадку и определяет.
func TestNameMatchesRejectsGenericStandIn(t *testing.T) {
	rejects := [][2]string{
		{"Pushka Mitino", "Pushka"},
		{"Pushka Brateevo", "Pushka"},
		{"Летний кинотеатр Фили", "Летний кинотеатр"},
		{"Космик Кинотеатр Рио", "Космик Кинотеатр"},
	}
	for _, c := range rejects {
		if nameMatches(c[0], c[1]) {
			t.Errorf("общее имя %q подставилось под уточнённое %q — площадки сольются в одну точку",
				c[1], c[0])
		}
	}

	// Имя из одних родовых слов не опознаётся вовсе: таких объектов в городе
	// несколько, и подставится любой.
	if nameMatches("Летний кинотеатр", "Летний кинотеатр") {
		t.Error("имя без различающих слов признано опознанным")
	}

	// А разные площадки одной сети между собой не путаются.
	if nameMatches("Pushka Mitino", "Pushka Brateevo") {
		t.Error("два филиала одной сети признаны одной площадкой")
	}
}
