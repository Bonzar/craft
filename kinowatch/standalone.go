...
var fixedChannels = []FixedChannel{
	...
	// (продолжение списка каналов; таблица псевдонимов — ниже, у venueAliases)
...
// ——— Псевдонимы вывесок ———
//
// Вторая вывеска — это когда агрегатор зовёт площадку одним именем, а реестр
// другим, и никакое правило разбора имени их не сведёт: это разные слова, а не
// разное написание. Такое нельзя угадывать — только записывать поимённо, с
// уликой и с решением Влада по каждой строке.
//
// Таблица адресует строку по eaisid, а не по имени: одинаковых имён в реестре
// восемь групп, и имя цели не идентифицирует.
//
// Расти «на всякий случай» таблица не имеет права: лишняя запись привяжет чужую
// площадку к чужой строке, и выглядеть это будет как честная привязка.
type venueAlias struct {
	// Title — как зовёт площадку агрегатор, дословно.
	Title string
	// EaisID — строка реестра, которую эта вывеска называет.
	EaisID string
	// Source — чем доказано. Без улики запись не заводится.
	Source string
}

var venueAliases = []venueAlias{
	{
		Title: "Киномакс Пражская", EaisID: "6101",
		Source: "api.kinomax.ru/rest/cinemas/prazhskaya — адрес из ответа: ул. Кировоградская, " +
			"д. 13А, ТРЦ «Columbus», 4 этаж; реестр зовёт ту же площадку «Киномакс - Колумбус»",
	},
	{
		Title: "Киносфера IMAX", EaisID: "2947",
		Source: "cinema5.ru/api/v1/cinemas?cinemaIds=20 — «Киносфера IMAX ТЦ «Капитолий» " +
			"Ленинградский», адрес из ответа: г. Москва, ул. Правобережная, д. 1, корп. Б; " +
			"реестр зовёт ту же площадку «Киносфера Москва», точка kinoafisha ложится туда же",
	},
}

// aliasEaisID — строка реестра, назначенная этой вывеске вручную.
//
// Сравнение идёт по нормализованному ПОЛНОМУ имени, без снятия сети: снятие
// превратило бы «Киномакс Пражская» в «пражская», а такое короткое слово
// назначать строке вручную опасно.
func aliasEaisID(title string) (string, bool) {
	want := normalizeName(title)
	for _, a := range venueAliases {
		if normalizeName(a.Title) == want {
			return a.EaisID, true
		}
	}
	return "", false
}
