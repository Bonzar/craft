...
// venuesWithoutRelease — площадки, чей репертуар НЕ кинопрокат.
//
// Проверка обязательна и делается по афише самой площадки, а не по её названию.
// Это не формальность: на той же группе три «музея» — Еврейский музей, Центр
// Зотов и ГЭС-2 — показали в афише «Непокой», то есть текущий прокат наравне с
// мультиплексами. Списать их по слову «музей» значило бы вычеркнуть из охвата
// работающие кинозалы.
//
// Проверено 04.08.2026 по афишам: у восьми ниже ни одного текущего релиза,
// вместо них выставки, лекции и купольные программы.
var venuesWithoutRelease = map[string]string{
	"музей кино":                 "афиша музейная, текущих релизов нет; билеты на события через TicketNet",
	"центральный музей кино":     "та же площадка второй строкой реестра: музейная афиша без проката",
	"дарвиновский":               "афиша из выставок и экскурсий, кинопроката нет",
	"культурный центр зил":       "выставки, лекции и фестивали; проката в афише нет",
	"дом русского зарубежья":     "выставочная и лекционная программа, проката нет",
	"музей az":                   "выставки и лекции, кинопроката нет",
	"планетарий":                 "купольные программы и лекции; «Смешарики» на сайте — встреча с гостями, а не показ",
	"центр документального кино": "расписания нет ни в разметке, ни после исполнения скриптов страницы",
	"театр на страстном":         "открытая сцена без репертуара кино: после снятия тегов слова «кино» в афише нет",

	// Пять строк ниже — решение ВЛАДА, а не вывод по афише: их источники не
	// отвечают ни напрямую, ни через российский туннель, проверить репертуар
	// нечем. Формулировка намеренно называет основание, чтобы через год это не
	// читалось как установленный факт.
	"сад эрмитаж":            "источник не отвечает, проверить репертуар нечем; выведено решением Влада",
	"филармония":             "источник не отвечает, проверить репертуар нечем; выведено решением Влада",
	"союз кинематографистов": "источник не отвечает, проверить репертуар нечем; выведено решением Влада",
	"стрелка":                "источник не отвечает, проверить репертуар нечем; выведено решением Влада",
	"театр марионеток":       "источник не отвечает, проверить репертуар нечем; выведено решением Влада",
}
