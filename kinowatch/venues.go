...
var venueBrands = []string{"каро под звездами", "каро", "киномакс", "москино",
	...
	// Та же сеть кириллицей. Реестр пишет «МОРИ Синема Кунцево» (6130), а
	// агрегаторы — латиницей; без обоих написаний площадка не находится, как
	// это уже было с «Кино Okko».
	"мори синема",
	// Агрегаторы пишут вывеску вместе с сетью — «Вики Синема ЗигЗаг», — а
	// реестр держит их врозь: имя «ЗигЗаг», сеть отдельным полем. Без снятия
	// ключи расходятся, и площадка уходит в «реестр не опознал», хотя строка
	// у неё есть. Координатная ступень тут не спасает: у строки 7458 точки нет.
	"вики синема",
	// Тот же случай: строки «Гудзон» (7784) и «Весна» (7761) несут сеть ЛЮКСОР
	// отдельным полем, а агрегаторы зовут их «Люксор Гудзон» и «Люксор Весна».
	"люксор"}
...
// venueKindHead — родовое слово «кинотеатр» в начале имени.
//
// Реестр пишет часть площадок с ним («Кинотеатр Киноквартал», 6309 и 6673;
// «Кинотеатр "МОРИ Синема Дрим"», 8906), агрегаторы — без него, и ключи
// расходятся на пустом месте.
//
// Снятие не имеет права обнулить ключ: площадка может так и называться.
var venueKindHead = regexp.MustCompile(`^кинотеатр\s+`)
...
func venueKey(name string) string {
	...
	if cut := strings.TrimSpace(venueKindHead.ReplaceAllString(s, "")); cut != "" {
		s = cut
	}
	brandCut := false
