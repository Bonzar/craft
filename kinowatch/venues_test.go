...
func TestVenueKeyStripsCinemaParkBrands(t *testing.T) {
	cases := []struct{ registry, directory string }{
		...
		// Реестр пишет вывеску кириллицей, справочник — латиницей и вперемешку.
		// На глаз три написания неразличимы, для сравнения строк — совершенно
		// разные, и без всех трёх площадка остаётся без канала.
		{"КИНО ОККО Афимолл Сити", "КИНО Okko Афимолл Сити"},
		{"КИНО ОККО Щёлковский", "Кино Оkkо Щёлковский"},
	}
	for _, c := range cases {
		...
		// Снятой вывески мало: ключи двух сторон обязаны СОВПАСТЬ, иначе
		// площадка так и останется без канала.
		if want := venueKey(c.directory); got != want {
			t.Errorf("ключи разошлись: %q → %q, %q → %q", c.registry, got, c.directory, want)
		}
	}
}
