...
// Ждём человека. Исходы видны ИЗ БРАУЗЕРА, потому что сетевого канала до пульта у
// песочницы нет:
...
//   «вкладка закрылась» — ждать больше нечего.
...
//
// СРОКА ПО УМОЛЧАНИЮ НЕТ, и это главное свойство функции. Машина не знает, когда человек
// освободится, а таймер превращал бы «ещё не пришёл» в «отказал» — два разных события с
// одним исходом. Ожидание кончается только ответом человека или закрытой вкладкой. Явный
// срок аргументом остаётся: он нужен замерам и коротким пробам.
var БЕЗ_СРОКА = 0;   // столько означает «без срока» у ожидания в браузере

function _закрыта(page) { try { return page.isClosed(); } catch (e) { return false; } }

async function owait(page, ms) {
  var предел = ms === undefined ? БЕЗ_СРОКА : ms;
  ...
  try {
    ...
    // Три разных повода выйти, и они НЕ схлопываются в один: вкладка закрылась — ждать
    // больше нечего; истёк явный срок замера — про это просили; всё прочее — поломка,
    // и выдавать её за «человек не ответил» нельзя.
    if (_закрыта(page)) { console.log("вкладка закрылась — ждать больше нечего"); return "вкладка закрылась"; }
    if (ms !== undefined) { console.log("истёк заданный замером срок"); return "нет"; }
    console.log("ожидание оборвалось: " + ((e && e.message) || e));
    return "оборвалось";
  }
}
