...
# Адрес узла берётся из env MAC_NODE_ADDR — в репозитории его нет.
...
# ПОСЛЕ подгрузки .env, иначе на локальной машине не увидит заведённую там переменную.
MAC_NODE="${MAC_NODE_ADDR:-}"             # tailnet-адрес MacBook; проверен именно адрес,
                                          # CLI принимает и имя, но тот путь не исполнялся
[[ -n "$MAC_NODE" ]] || die "нет MAC_NODE_ADDR в окружении — tailnet-адрес мака кладётся в настройки облачного окружения рядом с ключом Tailscale"
