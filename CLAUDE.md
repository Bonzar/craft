...
Правило действует и на хвост хода: любой tool-вызов ПОСЛЕ готового
отчёта (PushNotification рутины, прогрев индекса, любая «мелочь»)
съедает отчёт — до Влада дойдёт только текст, написанный после
последнего вызова. Порядок финала жёсткий: сперва все вызовы
инструментов, включая уведомление рутины, затем отчёт единственным
текстом — и после него ни одного вызова. Отсылка «отчёт выше» вместо
самого отчёта запрещена: «выше» для Влада не существует. Проверка
перед завершением хода: если после текста деливерабла был хоть один
вызов — повторить деливерабл целиком заново финальным сообщением.
(Инцидент 2026-07-20: отчёт гигиены умер из-за PushNotification,
поставленного после него.)
