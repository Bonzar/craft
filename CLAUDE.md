...
### Данные не отдаются простым фетчем — сразу поднимай браузер

Правило Влада: фраза «сайт боту не отдаёт» (про цену, характеристику,
наличие — любые данные, которые стоило бы добавить) без поднятого браузера
ЗАПРЕЩЕНА. WebFetch/curl вернул заглушку или пустоту → сразу запускай
headless Chromium по рецепту выше и пробуй через него — издержек у
запуска браузера нет. Только если и реальный браузер упёрся (Qrator-челлендж
Золотого Яблока, гео-блок RU-аптек и ритейла на зарубежном облачном IP,
капча поисковиков) — фиксируй данные как отсутствующие и называй Владу
конкретную причину, а не «не отдаёт».
...
- Яндекс-поиск в headless из этого окружения иногда проходит без капчи,
  но может встретить «Вы не робот?»; Google и DuckDuckGo — не работают.
