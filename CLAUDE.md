...
craft-sync — инструмент ЧТЕНИЯ, не записи: детект изменений для актуализации
памяти (снапшот и дифф) и индекс обратных ссылок (`--backlinks`). Команд
создания и правки блоков у него нет — только флаги (`-backlinks`, `-docs`,
`-incremental` и подобные), не подкоманды `blocks`. Запись и правка Craft
идут только через MCP-сервер Craft (`craft_write`) с `--json`; «blocks
add/update/learn» — это MCP, а не craft-sync.
